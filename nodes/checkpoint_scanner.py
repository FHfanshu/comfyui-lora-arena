...
class LoRArenaCheckpointScanner:
    ...
    def scan(self, directory: str):
        # If directory is empty, use config default
        scan_dir = directory.strip() if directory.strip() else self._load_config_lora_directory() or None
        with db_manager.session_scope() as db:
            result = checkpoint_service.scan_directory(db, scan_dir)
